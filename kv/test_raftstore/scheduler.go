...
import (
	...
	"miniLinkDB/proto/pkg/schedulerpb"
)
...
func (m *MockSchedulerClient) handleHeartbeatConfVersion(region *metapb.Region) error {
	...
	if region.RegionEpoch.ConfVer > searchRegion.RegionEpoch.ConfVer {
		// If ConfVer changed, LinkKV has added/removed one peer already.
		// So scheduler and LinkKV can't have same peer count and can only have
...
func (m *MockSchedulerClient) SetRegionHeartbeatResponseHandler(storeID uint64, h func(response *schedulerpb.RegionHeartbeatResponse)) {
	if h == nil {
