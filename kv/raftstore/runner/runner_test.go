...
func newEnginesWithKVDb(t *testing.T, kv *badger.DB) *engine_util.Engines {
	...
	engines.Kv = kv
	var err error
	engines.RaftPath, err = ioutil.TempDir("", "linkkv_raft")
	require.Nil(t, err)
...
func raftLogMustExist(t *testing.T, db *badger.DB, regionId, startIdx, endIdex uint64)  {
	for i := startIdx; i < endIdex; i++ {
		k := meta.RaftLogKey(regionId, i)
		db.View(func(txn *badger.Txn) error {
			item, err := txn.Get(k)
			assert.Nil(t, err)
