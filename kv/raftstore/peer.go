...
import (
	...
	"miniLinkDB/kv/raftstore/runner"
...
func replicatePeer(storeID uint64, cfg *config.Config, sched chan<- worker.Task,
	...
	log.Infof("[regioni %v] replicates peer with ID %d", regionID, metaPeer.GetId())
...
func NewPeer(storeId uint64, cfg *config.Config, engines *engine_util.Engines, region *metapb.Region, regionSched chan<-worker.Task, meta *metapb.Peer) (*peer, error) {
...
func (p *peer) Destory(engine *engine_util.Engines, keepData bool) error {
	start := time.Now()
	region := p.Region()
