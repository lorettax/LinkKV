...
func checkFileChecksum(path string, expectedChecksum uint32) error {
	...
	if checksum != expectedChecksum {
		return errors.Errorf("invalid checksum %d for snapshot cf file %s, expected %d", checksum, path, expectedChecksum)
	}
	return nil
}
...
func (s *Snap) validate() error {
	for _, cfFile := range s.CFFiles {
		if cfFile.Size == 0 {
			// Skip empty file. The checksum of this cf file should be 0 and this is checked when loading the snapshot meta.
			continue
		}
		// TODO: prepart and validate for ingestion
	}
	return nil
}
